...
namespace _ExercicioEFCoreCodeFirst
{
    class Program
    {
        static void Main(string[] args)
        {
            using (var db = new MovieContext())
            {
                ...
                Console.WriteLine("1) Todos os atores de Stars Wars: ");

                var actors = db.Characters
                    .Include("Movie")
                    .Include("Actor")
                    .Select(m => m)
                    .Where(m => m.Movie.Title.Equals("Star Wars"));

                foreach (ActorMovie actor in actors)
                {
                    Console.WriteLine("\t{0}", actor.Actor.Name);
                }

                Console.WriteLine();
                Console.WriteLine("2) Todos os atores que desemprenharam o papel de 007: ");
                ...
                {
                    Console.WriteLine("\t{0}", actor.Key);
                }

                Console.WriteLine();
                Console.WriteLine("3) Qual ator realizou mais filmes como o 'agente 007'? ");
